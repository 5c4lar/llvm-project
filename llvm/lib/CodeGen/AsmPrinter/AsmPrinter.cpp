...
void AsmPrinter::emitFunctionBody() {
  ...
  // 5c4lar
  auto &GtirbFunction = MAI->GtirbInfo.Functions.emplace_back();
  ...
  for (auto &MBB : *MF) {
    ...
    MCAsmInfo::MCGtirbInfo::Function::BasicBlock Block = {
        GtirbBBBeginSym, GtirbBBEndSym, MBB.isEntryBlock()};
...
bool AsmPrinter::doFinalization(Module &M) {
  ...
  for (const auto &G : M.globals()) {
    ...
    const DataLayout &DL = G.getDataLayout();
